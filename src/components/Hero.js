...
function Hero() {
  ...
  function handleShowHeroModal() {
    if (!showHeroModal) {
      setShowHeroModal(true);
      setTimeout(() => setIsVisible(true), 10); // Add slight delay to trigger transition
    } else {
      setIsVisible(false);
      setTimeout(() => setShowHeroModal(false), 300); // Delay to allow fade-out transition
    }
  }
  ...
  return (
    <section className="flex items-center hero">
      ...
      {showHeroModal && (
        <div
          className={`fixed inset-0 z-40 flex items-center justify-center min-h-screen transition-opacity duration-300 ${
            isVisible ? "opacity-100" : "opacity-0"
          }`}
        >
          <Modal handleShowHeroModal={handleShowHeroModal} />
        </div>
      )}
    </section>
  );
